...
describe("ResearchTransformService", () => {
  ...
  beforeEach(() => {
    ...
    service = new ResearchTransformService(
      prisma as never,
  ...
  it("normalizes mixed data into the v1 research pack without leaking pii", async () => {
    ...
    const measurementsCsv = result.repoFiles.find((file) => file.name === "research_measurements.csv");
    const appointmentsCsv = result.repoFiles.find((file) => file.name === "research_appointments.csv");
    const revocationsCsv = result.repoFiles.find((file) => file.name === "research_revocations.csv");
