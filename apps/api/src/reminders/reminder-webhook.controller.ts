...
export class ReminderWebhookController {
  ...
  async handleTwilioStatus(
    ...
    @Req() req: { headers: Record<string, string>; protocol: string; get: (name: string) => string | undefined; originalUrl: string }
  ) {
    ...
    if (mappedStatus) {
      await this.reminderService.updateDeliveryStatus(
        MessageSid,
        mappedStatus,
        body.ErrorCode
      );
  ...
  private validateTwilioSignature(
    req: { headers: Record<string, string>; protocol: string; get: (name: string) => string | undefined; originalUrl: string },
