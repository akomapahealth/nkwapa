...
export class PatientsController {
  ...
  @Get(':patientId')
  async findOne(@Param('patientId') patientId: string, @Query('clinicId') clinicId: string) {
    if (!clinicId) {
      throw new BadRequestException('clinicId query parameter is required');
    }
    const result = await this.patientService.findByIdWithRecentEncounters(patientId, 10, clinicId);
    if (!result) {
