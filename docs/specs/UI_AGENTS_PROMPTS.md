...
Once the UI is in, the next “make it clinic-ready” items are:
	1.	Reminder worker + Fake SMS provider (so follow-up isn’t just a date field)
	2.	Encounter state machine hardening (server-side enforcement)
	3.	Audit query filters + pagination
	4.	Conflict resolution UI improvements (duplicate patient resolution workflow)
